...
class ParserGUI:
	...
	def save_table(self, remove_index=None):
		"""Saves regular expressions and delimeters to a file"""
		table = self.get_table(remove_index)
	...
	def set_table(self, f):
		...
		for line in f:
			if line.strip() != "":
				...
				if self.xml:
					for j in range(4, min(7, len(values))):
	...
	def delete_row(self, row_index):
		self.save_table(remove_index=row_index)
		with open(self.TEMP_TABLE) as f:
			self.set_table(f)
		os.remove(self.TEMP_TABLE)
